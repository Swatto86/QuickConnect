<#
.SYNOPSIS
    QuickConnect Release Script - Creates versioned releases with proper Git tagging.

.DESCRIPTION
    This script automates the release process:
    1. Validates semantic version format (x.y.z)
    2. Updates version in Cargo.toml, tauri.conf.json, and about.html
...
function Update-AboutHtml {
    ...
    # Update the version shown in the About dialog
    $pattern = '(<p class="text-base-content/70 text-sm">Version )(\d+\.\d+\.\d+)(</p>)'
    $replacement = "`${1}$NewVersion`${3}"
    $updated = $content -replace $pattern, $replacement
...
Write-ColorOutput "           QuickConnect Release Script                      " "Cyan"
...
git add $CargoTomlPath $TauriConfPath $AboutHtmlPath
...
# Use --allow-empty for Force mode (re-releasing same version)
if ($Force) {
...
# Create annotated tag with release notes as the message
git tag -a $tagName -m $Notes
if ($LASTEXITCODE -ne 0) {
    Write-ErrorMsg "Failed to create tag"
    # Attempt to undo commit
...
Write-ColorOutput "  https://github.com/Swatto86/QuickConnect/actions" "Cyan"
Write-Host ""
